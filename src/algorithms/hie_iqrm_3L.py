...
from src.Agent.hie_iqrm_3L_agent import Agent, L1_Controller, L2_Controller
...
class Step_Counter:
    ...
    def is_episode_terminal(self):
        return (self.step >= self.max_episode_length)
...
def run_qlearning_task(tester,
    ...
    l2_controller.initialize_reward_machine()
    for l1_controller in l1_controller_list:
        l1_controller.initialize_reward_machine()
    ...
    while not (step_counter.is_episode_terminal() or env.is_terminal()):
        ...
        while not (step_counter.is_option_terminal() or env.is_terminal()):
            ...
            while not (step_counter.is_option_terminal() or env.is_terminal()):
                ...
                tester.add_step()
                is_l1_state_changed = l1_controller.update_controller(l)
                for other_controller in l1_controller_list:
                    if other_controller != l1_controller:
                        other_controller.update_controller(l)
                if is_l1_state_changed or tester.start_test():
                    break
...
def run_test(l2_controller,
    ...
    Parameters
    ----------
    l2_controller: L2 controller
    l1_controller_list: list of L1 controller
    agent_list : list of Agent objects
        Agent objects to be trained and tested.
    ...
    trajectory = []
    # steps = 0
    # max_episode_length = testing_params.num_steps
...
def run_hie_iqrm_3L_experiment(tester,
