...
from src.Agent.dqprm_s_agent import Agent
    ...
    for t in range(num_steps):
        ...
        for i in range(num_agents):
            ...
            if not (agent_list[i].is_task_complete):
                ...
                r, l, s_new = training_envs[i].environment_step(s, a)
                ...
                for u in agent_list[i].rm.U:
                    if not (u == current_u) and not (u in agent_list[i].rm.T):
                        l = training_envs[i].get_mdp_label(s, s_new, u)
                        r = 0
                        u_temp = u
                        u2 = u
                        for e in l:
        ...
        if testing_params.test and tester.get_current_step() % testing_params.test_freq == 0:
            ...
            testing_reward, trajectory, testing_steps = run_dqprm_test(agent_list_copy,
                                                                       tester,
                                                                       learning_params,
                                                                       testing_params,
...
def run_dqprm_experiment(tester,
                         num_times,
                         show_print=True):
    """
