...
def buttons_config(num_times):
    ...
    joint_rm_file = os.path.join(base_file_path, 'reward_machines', 'buttons', 'team_buttons_rm.txt')

    local_rm_files = []
    for i in range(num_agents):
        local_rm_string = os.path.join(base_file_path, 'reward_machines', 'buttons', 'buttons_rm_agent_{}.txt'.format(i + 1))
    ...
    learning_params.initial_epsilon = 0.1
    ...
    env_settings['p'] = 0.98  # noise action param
