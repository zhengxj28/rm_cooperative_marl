...
class Agent:
    ...
    def __init__(self, rm_file, num_states, actions, agent_id):
        """
    ...
    def get_next_action(self, s, epsilon, learning_params):
        ...
        if random.random() < epsilon:
            ...
            pr_sum = np.sum(np.exp(self.q[s, self.u, :] * T))
            pr = np.exp(self.q[s, self.u, :] * T) / pr_sum  # pr[a] is probability of taking action a
    ...
    def update_agent(self, s, a, reward, s_new, label, learning_params, update_q_function=True):
        ...
        if update_q_function:
            self.update_q_function(s, s_new, u_start, self.u, a, reward, learning_params)
    ...
    def update_q_function(self, s, s_new, u, u_new, a, reward, learning_params):
        ...
        if self.rm.is_terminal_state(u_new):
            self.q[s][u][a] = (1 - alpha) * self.q[s][u][a] + alpha * reward
        else:
            # Bellman update
            self.q[s][u][a] = (1 - alpha) * self.q[s][u][a] \
                                     + alpha * (reward + gamma * np.amax(self.q[s_new][u_new]))
